...
We can evaluate the best two models on the test set. It is preferable to create a split object using [`initial_split()`](https://www.tmwr.org/splitting.html#splitting-methods) and then apply the [`last_fit()`](https://www.tidymodels.org/start/case-study/#last-fit) function. However, since we have separate data frames, we'll use `augment()`: 
